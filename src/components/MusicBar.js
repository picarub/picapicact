...
export default class MusicBar extends React.Component {
 ...
 render(){
  ...
  return (
   <div style={{magin:'0 auto',maxWidth:'640px',textAlign:'center'}}>
     ...
     return(
      <div key={'_tr_'+ i} className='visible-xs rounded animated flipInX' style={{backgroundColor:'#fff',margin:'.6em',boxShadow:'.2em .2em .4em #c5c5c5'}}>
       <div className='row' style={{alignItems:'center'}}> 
        <div className='col-3 sm-2 hvr-grow' style={{position:'relative', margin:'0 0 -.3em'}} onClick={()=>{playplay(ea.url, i)}}>
        ...
        <div className='col-9 sm-10 left' style={{padding:'0 1em'}}>
         <div className='col-12' style={{fontWeight:'bolder',fontSize:'1.1em'}}><a style={{color:'#000'}} target='_blank' href={ea.songUrl}>{ea.songName}</a></div>
         <div className='col-12' style={{whiteSpace: 'nowrap',textOverflow: 'ellipsis',overflow: 'hidden'}}><a style={{color:'#666'}} target='_blank' href={ea.artistUrl}>{ea.artist}</a></div>
         <div className='col-12' style={{color:'#aaa',whiteSpace: 'nowrap',textOverflow: 'ellipsis',overflow: 'hidden'}}>{ea.albumName}</div>
        </div>
       </div>
      </div>
     )  
    ...
    {data.map((ea,i) => {
     return(
      <div key={'_tr_'+ i} className='col-4 invisible-xs animated flipInX' style={{padding:'.6em'}}>
